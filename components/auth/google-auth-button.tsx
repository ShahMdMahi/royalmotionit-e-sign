...
export function GoogleAuthButton({
    ...
    startTransition(async () => {
      try {
        ...
        if (redirectUrl) {
          ...
          toast.error(
            "Failed to initiate Google authentication. No redirect URL returned.",
          );
        ...
        if (error instanceof Error) {
          ...
          toast.error(
            `Google authentication error: ${error.message || "Unknown error"}`,
          );
  ...
  return (
    <Button
      type="button"
      variant={variant}
      className={`h-8 text-sm ${fullWidth ? "w-full" : ""} ${className}`}
