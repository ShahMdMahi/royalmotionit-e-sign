    ...
    <div className="container mx-auto py-10 max-w-7xl">
      ...
      <Tabs defaultValue={defaultTab} className="w-full">
        <div className="mb-8 border-b border-border/80 overflow-x-auto">
          <TabsList className={`grid w-full ${isAdmin ? "grid-cols-2" : "grid-cols-1"} md:w-auto justify-start bg-transparent h-12`}>
            {isAdmin && (
              <TabsTrigger value="authored" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
                <FileSignature className="size-4" />
                Authored Documents
              </TabsTrigger>
            )}
            <TabsTrigger value="signees" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
              ...
              <CardContent className="p-6 space-y-8">
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {authorDocuments.map((doc) => (
                    <Card key={doc.id} className="group border-border shadow-sm transition-all duration-300 hover:shadow-xl hover:border-primary/50 hover:-translate-y-1">
                      <CardHeader className="pb-3 space-y-3">
                        <div className="flex justify-between items-start gap-3">
                          <CardTitle className="truncate text-lg font-semibold group-hover:text-primary transition-colors">{doc.title}</CardTitle>
                          <Badge
                            variant={doc.status === "PENDING" ? "outline" : doc.status === "APPROVED" ? "default" : "secondary"}
                      ...
                      <CardContent className="pb-3 space-y-4">
                        <div className="flex items-center gap-2.5 text-sm">
                          ...
                          <span className="text-muted-foreground font-medium">{doc.documentType}</span>
                        </div>
                        ...
                        <div className="bg-muted/5 rounded-lg p-3 space-y-2.5">
                          ...
                          {doc.preparedAt && (
                            <div className="flex items-center justify-between text-xs">
                              <span className="text-muted-foreground flex items-center gap-1.5">
                                <FileSignature className="h-3.5 w-3.5" /> Prepared:
                              </span>
                ...
                {signeeDocuments.map((doc) => (
                  <Card key={doc.id} className="group border-border shadow-sm transition-all duration-300 hover:shadow-xl hover:border-primary/50 hover:-translate-y-1">
                    <CardHeader className="pb-3 space-y-3">
                      <div className="flex justify-between items-start gap-3">
                        <CardTitle className="truncate text-lg font-semibold group-hover:text-primary transition-colors">{doc.title}</CardTitle>
                        <Badge variant={doc.documentType === DocumentType.UNSIGNED ? "outline" : "default"} className="shrink-0 shadow-sm uppercase text-[10px] px-3 py-0.5 font-bold tracking-wider">
