    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onCloseAction()}>
      <DialogContent className="sm:max-w-md max-w-[90vw] w-full mx-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className="text-lg xs:text-xl">Upload Document</DialogTitle>
            <DialogDescription className="text-xs xs:text-sm">Upload a document for signature. Supported format: PDF (max 10MB).</DialogDescription>
          </DialogHeader>
            ...
            <div className="border rounded-lg p-2 xs:p-3 sm:p-4 my-3 xs:my-4">
              <div className="flex items-center gap-2 xs:gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-xs xs:text-sm truncate">{file.name}</p>
                  <p className="text-[10px] xs:text-xs text-muted-foreground">{(file.size / 1024 / 1024).toFixed(2)} MB • PDF</p>
                </div>
                <Button type="button" variant="ghost" size="icon" onClick={removeFile} className="size-6 xs:size-7 sm:size-8 rounded-full">
          ...
          {uploading && (
            <div className="my-3 xs:my-4">
              <div className="flex justify-between items-center mb-1.5 xs:mb-2">
                <span className="text-xs xs:text-sm font-medium">Uploading...</span>
                <span className="text-xs xs:text-sm text-muted-foreground">{uploadProgress}%</span>
              </div>
          ...
          <div className="grid gap-3 xs:gap-4 py-2">
            <div className="grid gap-1.5 xs:gap-2">
              ...
              <Input id="fileName" placeholder="Enter document title" value={fileName} onChange={(e) => setFileName(e.target.value)} disabled={uploading} required className="h-8 xs:h-9" />
            </div>
            <div className="grid gap-1.5 xs:gap-2">
          ...
          <DialogFooter className="mt-3 xs:mt-4 flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2">
            ...
            <Button type="submit" disabled={uploading || !file || !fileName.trim()} className="w-full sm:w-auto">
