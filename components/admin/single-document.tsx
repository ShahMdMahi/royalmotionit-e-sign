...
export function SingleDocumentComponent({
  ...
  if (isLoading) {
    return (
      <Card className="w-full max-w-7xl mx-auto my-4 sm:my-8 border-border card-hover shadow-lg">
        <CardHeader className="border-b border-border p-4 sm:p-6">
          <CardTitle className="text-lg xs:text-xl sm:text-2xl font-semibold flex items-center gap-2">
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col justify-center items-center h-64 sm:h-96 space-y-3 sm:space-y-4">
  ...
  if (error && !pdfData) {
    return (
      <Card className="w-full max-w-7xl mx-auto my-4 sm:my-8 border-border card-hover shadow-lg">
        <CardHeader className="border-b border-border bg-destructive/5 p-4 sm:p-6">
          <CardTitle className="text-lg xs:text-xl sm:text-2xl font-semibold flex items-center gap-2">
            <div className="size-6 sm:size-8 rounded-full bg-destructive/10 flex items-center justify-center">
              <AlertTriangle className="size-3 sm:size-4 text-destructive" />
            </div>
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col justify-center items-center h-64 sm:h-96 text-destructive space-y-3 sm:space-y-4">
            <div className="p-3 sm:p-4 border border-destructive/50 bg-destructive/10 rounded-md max-w-md w-full">
              <p className="flex items-start gap-2 text-xs xs:text-sm">
                <AlertTriangle className="size-3 sm:size-4 shrink-0 mt-1" />{" "}
                {error}
              </p>
            </div>
            <Button
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <Card className="w-full max-w-7xl mx-auto my-4 sm:my-8 border-border card-hover shadow-lg">
        <CardHeader className="border-b border-border p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 sm:items-center justify-between">
            ...
            <Badge
              ...
              className="text-xs sm:text-sm px-2 py-1 h-auto self-start sm:self-auto mt-1 sm:mt-0"
            >
        ...
        <CardContent className="p-0">
          <Tabs defaultValue="document" className="w-full">
            ...
            <TabsContent value="document" className="p-0 border-t">
              {error && pdfData && (
                <div className="m-4 sm:m-6 p-3 sm:p-4 border border-destructive/50 bg-destructive/10 text-destructive rounded-md">
                  <p className="flex items-center gap-2 text-xs xs:text-sm">
                    <AlertTriangle className="size-3 sm:size-4" /> There was an
                    issue refreshing the document: {error}. Displaying cached or
                    previous version.
                  </p>
                </div>
              )}
              ...
              {!pdfData && !error && (
                <div className="flex flex-col items-center justify-center h-64 sm:h-96 border-b m-4 sm:m-6 rounded-md bg-muted/20">
                  <FileText className="size-8 sm:size-12 text-muted-foreground/50 mb-2" />
                  <p className="text-xs xs:text-sm text-muted-foreground">
              ...
              <ScrollArea className="h-full">
                <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    <Separator />
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-3 sm:gap-y-4 text-xs xs:text-sm rounded-lg bg-muted/20 p-3 sm:p-4">
                      <div>
                        <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                          <Hash className="size-3 sm:size-3.5" /> Document ID:
                        </p>
                        <p className="text-muted-foreground break-all bg-background/50 p-1 sm:p-1.5 rounded border text-[10px] xs:text-xs font-mono">
                          {document.id}
                        </p>
                      </div>
                      <div>
                        <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                          <FileText className="size-3 sm:size-3.5" /> File Name:
                        </p>
                        <p className="text-muted-foreground bg-background/50 p-1 sm:p-1.5 rounded border text-[10px] xs:text-xs">
                          {document.fileName || "N/A"}
                        </p>
                      </div>
                      <div>
                        <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                          <FileSignature className="size-3 sm:size-3.5" />{" "}
                          Title:
                        </p>
                        <p className="text-muted-foreground bg-background/50 p-1 sm:p-1.5 rounded border text-[10px] xs:text-xs">
                          {document.title}
                      ...
                      <div>
                        <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                          <Shield className="size-3 sm:size-3.5" /> Document
                          Type:
                        </p>
                        <p className="text-muted-foreground bg-background/50 p-1 sm:p-1.5 rounded border text-[10px] xs:text-xs">
                      ...
                      {document.description && (
                        <div className="sm:col-span-2">
                          <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                            <Info className="size-3 sm:size-3.5" /> Description:
                          </p>
                          <p className="text-muted-foreground whitespace-pre-wrap bg-background/50 p-1 sm:p-1.5 rounded border text-[10px] xs:text-xs">
                            {document.description}
                          </p>
                        </div>
                      )}
                      <div>
                        <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                          <Shield className="size-3 sm:size-3.5" /> Status:
                        </p>
                        <Badge
                          variant={getStatusBadgeVariant(document.status)}
                          className="mt-1"
                      ...
                      <div className="sm:col-span-2">
                        <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                          <Clock className="size-3 sm:size-3.5" /> Timestamps:
                        </p>
                      ...
                      {document.signers && document.signers.length > 0 && (
                        <div className="sm:col-span-2">
                          <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                            <FileSignature className="size-3.5" /> Signer
                            Details:
                          </p>
                      ...
                      {document.hash && (
                        <div className="sm:col-span-2">
                          <p className="font-semibold text-primary flex items-center gap-1 sm:gap-1.5 mb-1">
                            <Hash className="size-3 sm:size-3.5" /> File Hash
                            (SHA256):
                          </p>
                          <p className="text-muted-foreground break-all text-[10px] xs:text-xs font-mono bg-background/50 p-1 sm:p-1.5 rounded border">
                            {document.hash}
                          </p>
                        </div>
                      )}
        ...
        <CardFooter className="border-t p-3 sm:p-4 flex justify-end gap-2 sm:gap-3">
          {document.documentType === "UNSIGNED" && (
            <Button size="sm" className="h-7 sm:h-8 text-xs sm:text-sm">
              ...
              >
                <Edit className="size-3 sm:size-4 mr-1 sm:mr-2" />
                Edit
              </Link>
            </Button>
          )}
