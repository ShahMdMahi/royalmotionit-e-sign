...
export function PDFViewerSimple({
  ...
  // Detect mobile devices - define early to avoid initialization issues
      ...
      const getFieldDisplay = () => {
        switch (field.type) {
          case "signature":
            return hasValue ? (
              <div className="absolute inset-0 flex items-center justify-center overflow-hidden w-full h-full">
                <Image src={value} alt="Signature" className="max-w-full max-h-full object-contain" draggable="false" />
              </div>
            ) : (
          ...
          case "initial":
            return hasValue ? (
              <div className="absolute inset-0 flex items-center justify-center overflow-hidden w-full h-full">
                <Image src={value} alt="Initial" className="max-w-full max-h-full object-contain" draggable="false" />
              </div>
            ) : (
        ...
        {(!workerLoaded || pdfLoading) && (
          <div className="absolute inset-0 flex items-center justify-center bg-background/80 z-50">
            <div className="flex flex-col items-center space-y-2">
              ...
              <div className="text-sm text-muted-foreground">{!workerLoaded ? "Loading PDF engine..." : "Loading document..."}</div>
            </div>
          </div>
        )}
        ...
        {pdfError && (
          <div className="absolute inset-0 flex items-center justify-center bg-background/80 z-50">
            <div className="text-red-500 text-center p-4 max-w-md">
              ...
              {retryCount >= maxRetries && <div className="text-xs text-muted-foreground">Maximum retry attempts reached. Please refresh the page.</div>}
            </div>
          </div>
        )}
            ...
            {canRenderPdf && (
              ...
              >
                ...
                >
                  ...
                  {/* Field overlays */}
                  {viewerLoaded && !pdfLoading && !pdfError && currentPageFields.map((field) => renderFieldOverlay(field))}
